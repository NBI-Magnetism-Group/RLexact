...
/* NB! NB! "j" changed by "new_state". Possible error in FCC32 calc. ??! */
    ...
    case FCC32MIRROR: /* Mirror reflection */
      new_state=0;
      for(j=0;j<32;j++) 
        if(bitmap&(1<<j)) new_state += 1<<M0[j];
      return new_state;
    case FCC32R2: /* Two-fold rotation */
      new_state=0;
      for(j=0;j<32;j++) 
        if(bitmap&(1<<j)) new_state += 1<<R2[j];
      return(new_state);
    case FCC32R4: /* Four-fold rotation */
      new_state=0;
      for(j=0;j<32;j++) 
        if(bitmap&(1<<j)) new_state += 1<<R4[j];
      return(new_state);
    case FCC32R3: /* Three-fold rotation */
      new_state=0;
      for(j=0;j<32;j++) 
        if(bitmap&(1<<j)) new_state += 1<<R3[j];
      return(new_state);
...
unsigned long long tmp, start=SPIN_0_UP;
  ...
  if(flipmask==0) flipmask=~flipmask;
#ifdef TEST_SYM
