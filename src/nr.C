...
long long **imatrix(long long ml, long long mh, long long nl, long long nh)
{
  ...
  if (im == NULL)
    nrerror("Allocation error 1 in imatrix()\n");
  im -= ml;
  ...
  for (i = ml; i <= mh; i++)
  {
    ...
    if (im[i] == NULL)
      nrerror("Allocation error 2 in imatrix()\n");
    im[i] -= nl;
  }
...
float **matrix(long long ml, long long mh, long long nl, long long nh)
{
  ...
  for (i = ml; i <= mh; i++)
  {
    ...
    if (!dm[i])
      nrerror("Allocation error 2 in matrix\n");
    dm[i] -= nl;
  }
...
double **dmatrix(long long ml, long long mh, long long nl, long long nh)
{
  ...
  if (dm == NULL)
    nrerror("Allocation error 1 in dmatrix()\n");
  dm -= ml;
  ...
  for (i = ml; i <= mh; i++)
  {
    ...
    if (dm[i] == NULL)
      nrerror("Allocation error 2 in dmatrix()");
    dm[i] -= nl;
  }
...
{ //(kvector = cvector as per RLexact.h)
  komplex *cv;

  cv = (komplex *)malloc((unsigned)(nh - nl + 1) * sizeof(komplex));
  if (cv == NULL)
    nrerror("Allocation error in cvector()\n");
  cv -= nl;
  return (cv);
}
...
komplex **cmatrix(long long ml, long long mh, long long nl, long long nh) // cmatrix = kmatrix
{
  ...
  if (cm == NULL)
    nrerror("Allocation error 1 in cmatrix()\n");
  cm -= ml;
  ...
  for (i = ml; i <= mh; i++)
  {
    ...
    if (cm[i] == NULL)
      nrerror("Allocation error 2 in cmatrix()");
    cm[i] -= nl;
  }
